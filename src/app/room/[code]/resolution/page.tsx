...
export default function ResolutionPage({ params }: { params: Promise<{ code: string }> }) {
  ...
  useEffect(() => {
    ...
    const loadResolutionData = async () => {
      // 1. Get Room to find the prompt ID
      ...
      // 2. Get the Prompt content (for the Steal attempt)
      ...
      if (foundImposter) {
        ...
        // Imposter is caught ONLY if they have the STRICT majority (Tie = Imposter survives)
    ...
    // Listen for the global phase change back to "lobby"
  ...
  // Execute the Client-Side Levenshtein Check
  const handleStealAttempt = async () => {
    ...
    // Check distance against all valid synonyms. Allowing a max distance of 2 for typos.
    ...
    // Push the final state to the server to apply scores and reset the room
  ...
  const handleHostContinue = async () => {
    ...
    setIsFinalizing(true);
    // If they weren't caught, they don't steal. Just finalize.
  ...
  return (
    <div className="flex flex-col h-full p-6 text-center">
      ...
      <div className="mt-8 space-y-4 border-b-4 border-fleshy-pink pb-8">
        ...
        <h1 className="font-display text-6xl text-warning-yellow drop-shadow-chunky leading-tight">
          {imposter.player_name}
        </h1>
        {isCaught ? (
          <div className="bg-toxic-green text-bruise-purple font-display text-4xl py-2 px-4 rounded-xl shadow-chunky inline-block transform rotate-2">
            CAUGHT!
          </div>
        ) : (
          <div className="bg-fleshy-pink text-white font-display text-4xl py-2 px-4 rounded-xl shadow-chunky inline-block transform -rotate-2">
            ESCAPED!
          </div>
        )}
      </div>

      {/* The Steal Phase (Only if caught) */}
          ...
          {isMeImposter ? (
            <>
              ...
              <button
                ...
                className="w-full bg-fleshy-pink text-white font-display text-4xl py-4 rounded-xl shadow-chunky active:translate-y-1 active:shadow-none border-4 border-bruise-purple disabled:opacity-50"
              >
      ...
      {(stealResult !== "pending" || !isCaught) && (
        <div className="flex-grow flex flex-col justify-center gap-6 mt-8">
          ...
          {/* Only the Host can click continue to reset the room, but we'll show it safely */}
          {players.find(p => p.id === playerId)?.id === players.find(p => p.room_code === code)?.id /* Quick host check proxy since we didn't fetch room host_id locally for non-caught states */ && (
             <button
             onClick={handleHostContinue}
             disabled={isFinalizing}
             className="mt-8 w-full bg-toxic-green text-bruise-purple font-display text-4xl py-4 rounded-xl shadow-chunky active:translate-y-1 active:shadow-none border-4 border-bruise-purple disabled:opacity-50"
           >
