  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        ...
        () => {
          router.replace("/"); // Host nuked the room
        }
      )
      .subscribe();
  ...
  if (errorMsg) {
    return (
      <div className="flex flex-col items-center justify-center h-full p-6 text-center space-y-6">
        <h1 className="font-display text-5xl text-warning-yellow drop-shadow-chunky text-balance">ROOM CLOSED</h1>
      ...
      <div className="flex flex-col h-full p-6 relative">
        ...
        <div className="flex-grow flex flex-col gap-4 overflow-y-auto pb-4">
          <h2 className="font-display text-3xl text-fleshy-pink border-b-4 border-fleshy-pink pb-2 flex justify-between items-center">
            ...
            {isHost && (
              <button onClick={() => setShowSettings(!showSettings)} className="text-sm bg-white text-bruise-purple px-2 py-1 rounded shadow-chunky transition-transform active:scale-90">
                SETTINGS
              </button>
            )}
          </h2>
          ...
          {sortedPlayers.map((player, idx) => (
            <motion.div 
              ...
              className={`p-4 rounded-xl border-4 shadow-chunky flex justify-between items-center ${
                player.id === currentPlayerId ? "bg-toxic-green border-bruise-purple text-bruise-purple shadow-chunky" : "bg-white border-bruise-purple text-bruise-purple"
              }`}
            >
        ...
        <AnimatePresence>
          {showSettings && isHost && (
            <motion.div initial={{ opacity: 0, y: 100 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 100 }} className="absolute inset-x-6 bottom-24 bg-bruise-purple border-4 border-warning-yellow p-6 rounded-2xl z-50 shadow-2xl space-y-4">
              <h3 className="font-display text-3xl text-warning-yellow text-center">GAME CONFIG</h3>
              <div className="flex gap-2">
                <button onClick={() => handleUpdateSettings('rounds', 5)} className={`flex-1 py-2 font-bold rounded-lg border-2 ${room.round_settings.mode === 'rounds' ? 'bg-warning-yellow text-bruise-purple border-white' : 'border-warning-yellow'}`}>ROUNDS</button>
                <button onClick={() => handleUpdateSettings('score', 10)} className={`flex-1 py-2 font-bold rounded-lg border-2 ${room.round_settings.mode === 'score' ? 'bg-warning-yellow text-bruise-purple border-white' : 'border-warning-yellow'}`}>SCORE</button>
              </div>
              <div className="flex items-center justify-between bg-dark-void p-3 rounded-xl border-2 border-white/20">
                <button onClick={() => handleUpdateSettings(room.round_settings.mode, Math.max(1, room.round_settings.target - 1))} className="text-4xl font-display text-fleshy-pink">-</button>
                <span className="font-display text-4xl">{room.round_settings.target}</span>
                <button onClick={() => handleUpdateSettings(room.round_settings.mode, room.round_settings.target + 1)} className="text-4xl font-display text-toxic-green">+</button>
              </div>
              <button onClick={handleNukeRoom} className="w-full bg-red-600 text-white font-display text-2xl py-2 rounded-xl border-4 border-bruise-purple shadow-chunky">NUKE LOBBY</button>
              <button onClick={() => setShowSettings(false)} className="w-full text-white/50 font-bold uppercase text-xs">Close</button>
            </motion.div>
          )}
        </AnimatePresence>
          ...
          {isGameOver ? (
            <div className="space-y-4">
              <div className="text-center animate-bounce">
                <p className="font-display text-5xl text-toxic-green shadow-chunky">WINNER: {sortedPlayers[0].player_name}</p>
              </div>
              {isHost && (
                <button onClick={handlePlayAgain} className="w-full bg-warning-yellow text-bruise-purple font-display text-4xl py-4 rounded-xl shadow-chunky border-4 border-bruise-purple transition-transform active:scale-95">PLAY AGAIN</button>
              )}
            </div>
          ) : isHost ? (
            <button 
              ...
              className={`w-full font-display text-4xl py-4 rounded-xl border-4 border-bruise-purple transition-all ${canStart && !isStarting ? "bg-toxic-green text-bruise-purple shadow-chunky active:translate-y-1 active:shadow-none" : "bg-gray-500 text-gray-300 opacity-50 cursor-not-allowed"}`}
            >
